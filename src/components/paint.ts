...
export default class Paint implements Component {
	...
	cropWorker = new Worker(
		new URL("./assets/crop.worker.ts", import.meta.url)
	);
		...
		if (this.currentTool) {
			...
			if (this.currentTool.movable) {
				...
				if (width && height) {
					const x = Math.round(
						Math.min(
							this.currentTool.lastX,
							this.currentTool.startX
						)
					);
					const y = Math.round(
						Math.min(
							this.currentTool.lastY,
							this.currentTool.startY
						)
					);
